...
SP_MODE_INSTRUCTIONS = [
    [
        "x", "Split at the current frame exclusively",
    ...
    [
        "o*2", "Remove all bounding boxes in this video",
        "Applies to whole video, not just the current section"
    ],
]
...
class SPGUIManager(VideoPlayerGUIManager):
    ...
    def get_return_fields(self) -> (List[Section], List[BBFields]):
        ...
        bbfs = []
        for sec in sections:
            new_bbs = self.bbm.bbox_fields.clone()
            new_bbs.crop_range(sec.start, sec.end)
            bbfs.append(new_bbs)
...
class SPMode(InternalMode):
    ...
    def handle_keyboard(self, key_mapper: KeyMapper):
        ...
        if key_mapper.consume("x"):
            ...
            if curr_tags is None:
                ...
                self.log("Removing all enum tags (previously {0})".format(
                    curr_tags))
    ...
    def get_state_message(self):
        return [
            "Split Mode",
            "{0}/{1} Deleted".format(self.sm.get_n_deleted(),
                                     self.sm.get_n_sections()),
    ...
    def modify_frame(self, frame, i):
        frame = self.sm.modify_frame(frame, i, self.par.vcm.get_width(),
                                     self.par.vcm.get_height())
        return frame
...
class Section(object):
    ...
    def split_failure_msg(self, loc):
        if abs(loc - self.start) <= 0:
            return "Split location too close to a previous split at {0}".format(
                self.start - 1)
        if abs(loc - self.end) <= 1:
            return "Split location too close to next split ar {0}".format(
                self.end)
        return None
    ...
    def __repr__(self):
        return f"<Secton {self.status}: ({self.start},{self.end})>"
    ...
    def join(cls, s1: Section, s2: Section) -> Section:
        return Section(min(s1.start, s2.start), max(s1.end, s2.end), s1.status,
                       s1.enum_tags.copy())
...
class SplitManager(object):
    ...
    def __init__(self, parent: SPMode, frame_count: int,
                 initial_enum_tags: List[str]):
        self.par = parent
    ...
    def split(self, loc):
        ...
        if self.secs[ii].status == SectionStatus.DELETED:
            self.par.warn(
                "Splitting on a deleted section. Both sides will be deleted")
    ...
    def erase_split(self, loc):
        ...
        info = self.__find(loc)
        if info.kind != SMLocInfo.SPLIT:
            return self.par.error(
                "Frame {0} is not the location of a split".format(loc))
        ii = info.ii
    ...
    def toggle_section(self, loc):
        ...
        self.par.log("Current section state was toggled to {0}".format(
            self.secs[info.ii].status))
    ...
    def modify_frame(self, frame, loc, width, height):
        def get_location(text, center_pct, formatter):
            textsize = cv2.getTextSize(text, formatter["fontFace"],
                                       formatter["fontScale"],
        ...
        if loc == 0 or kind == SMLocInfo.SECTION:
            ...
            if ii != 0:
                to_display.append(
                    DisplayParams("< " + str(loc - sec.start + 1), 0, 0.0,
                                  self.SPLIT_DISPLAY))
                to_display.append(
                    DisplayParams("At " + str(sec.start + 1), 1, 0.0,
                                  self.SPLIT_DISPLAY))

            # Current text
            to_display.append(
                DisplayParams("Section {0}".format(ii + 1), 0, 0.5,
                              f_by_status(self.secs[ii].status)))
            to_display.append(
                DisplayParams(str(self.secs[ii].enum_tags), 1, 0.5,
                              self.ENUM_DISPLAY))
            ...
            if ii <= len(self.secs) - 2:
                to_display.append(
                    DisplayParams(
                        str(sec.end - loc) + " >", 0, 1.0, self.SPLIT_DISPLAY))
                to_display.append(
                    DisplayParams("At " + str(sec.end), 1, 1.0,
                                  self.SPLIT_DISPLAY))
            ...
            if ii >= 1:
                to_display.append(
                    DisplayParams("< " + str(loc - self.secs[ii - 1].end), 0,
                                  0.0, self.SPLIT_DISPLAY))
                to_display.append(
                    DisplayParams("At " + str(self.secs[ii - 1].end), 1, 0.0,
                                  self.SPLIT_DISPLAY))

            # Current text
            to_display.append(
                DisplayParams("Split {0}".format(ii), 0, 0.5,
                              self.SPLIT_DISPLAY))
            ...
            if ii <= len(self.secs) - 3:
                to_display.append(
                    DisplayParams(
                        str(self.secs[ii + 1].end - loc) + " >", 0, 1.0,
                        self.SPLIT_DISPLAY))
                to_display.append(
                    DisplayParams("At " + str(self.secs[ii + 1].end), 1, 1.0,
                                  self.SPLIT_DISPLAY))
    ...
    def validate(self):
        ...
        if sum([sec.end - sec.start + 1
                for sec in self.secs]) != self.frame_count + 1:
            raise ValueError("Total number of frames is not correct")

        if len(set([id(sec.enum_tags)
                    for sec in self.secs])) != len(self.secs):
